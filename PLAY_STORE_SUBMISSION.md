...
- [ ] Phone screenshots — min 2, recommend 4-6, PNG/JPG, aspect ratio between
      16:9 and 9:16 (longer side ≤ 2× shorter side). **Not satisfied yet.** The
      four images in `assets/playstore_screenshots/`, generated by
      `PlayStoreScreenshotTest`, are 1078×2399 (≈2.23:1) — still too elongated,
      so Play will reject them. The test renders at `.size(412.dp, 915.dp)` on
      `RobolectricDeviceQualifiers.Pixel8`; at that density 915dp maps to
      2399px. Height needs to come down to ≈822dp (≈2156px) or lower to get
      under 2.0:1 — check the layout still composes sensibly at that height
      rather than just cropping it.
